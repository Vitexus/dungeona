...
texture parser from ``ans.py``. It keeps a medium-resolution internal 3D
view for speed, and uses pygame's display scaling path when GPU presentation
is requested instead of the experimental ``pygame._sdl2.video`` renderer.
...
Z / C             alternate strafe left / right
...
try:  # pragma: no cover - optional fast path
    import numpy as np
except Exception:  # pragma: no cover - keep pygame frontend usable without numpy
...
KEY_HELP = "Mouse look  WASD move  Arrows/QE turn  Space act  . wait  M map  </> stairs  Tab mouse  Esc/F12 quit"
...
class Dungeona3:
    def __init__(
        ...
        *,
        ...
        self.display_flags, self.display_vsync = self.compute_display_mode(self.display_flags, self.use_gpu_requested)
        self.display_surface = self.create_display_surface(self.window_width, self.window_height)
        pygame.display.set_caption("Dungeona3")
        self.clock = pygame.time.Clock()
        self.gpu_active = bool(self.display_vsync)
        ...
        self.scaled_view_surface: Optional["pygame.Surface"] = None
        self.screen: Optional["pygame.Surface"] = None
        self.internal_render_scale = 0.625
        self.internal_render_min = (400, 240)
        self.internal_render_max = (960, 540)

        self.texture_fill_cache: Dict[Tuple[object, ...], Tuple[Tuple[Optional[RGB], ...], ...]] = {}
        self.texture_rgb_cache: Dict[Tuple[object, ...], object] = {}
        self.texture_mask_cache: Dict[Tuple[object, ...], object] = {}
        self.trimmed_texture_cache: Dict[Tuple[object, ...], Tuple[Tuple[AnsiCell, ...], ...]] = {}
        self.vertical_sample_cache: Dict[Tuple[int, int], object] = {}
        self.screen_x_cache: Dict[int, object] = {}
        self.camera_x_cache: Dict[int, object] = {}
    ...
    def resize(self, width: int, height: int) -> None:
        ...
        view_width = max(1, self.view_rect[2])
        view_height = max(1, self.view_rect[3])
        internal_width = int(round(view_width * self.internal_render_scale))
        internal_height = int(round(view_height * self.internal_render_scale))
        self.view_width_cells = max(self.internal_render_min[0], min(view_width, self.internal_render_max[0], internal_width))
        self.view_height_cells = max(self.internal_render_min[1], min(view_height, self.internal_render_max[1], internal_height))
        self.view_surface = pygame.Surface((self.view_width_cells, self.view_height_cells)).convert()
        self.scaled_view_surface = pygame.Surface((view_width, view_height)).convert()
    ...
    def compute_display_mode(self, base_flags: int, use_gpu: bool) -> Tuple[int, int]:
        flags = base_flags
        vsync = 0
        if use_gpu and hasattr(pygame, "SCALED"):
            flags |= pygame.SCALED
            vsync = 1
        return flags, vsync

    def create_display_surface(self, width: int, height: int) -> "pygame.Surface":
        try:
            return pygame.display.set_mode((width, height), self.display_flags, vsync=self.display_vsync)
        except TypeError:
            return pygame.display.set_mode((width, height), self.display_flags)
        except pygame.error:
            if self.display_flags & getattr(pygame, "SCALED", 0):
                safe_flags = self.display_flags & ~getattr(pygame, "SCALED", 0)
                self.display_flags = safe_flags
                self.display_vsync = 0
    ...
    def texture_rgb_arrays(self, texture: Optional[AnsiTexture]) -> Tuple[object, object]:
        texture_id = self.texture_identity(texture)
        if texture_id is None or texture is None or np is None:
            return None, None

        rgb_cached = self.texture_rgb_cache.get(texture_id)
        mask_cached = self.texture_mask_cache.get(texture_id)
        if rgb_cached is not None and mask_cached is not None:
            return rgb_cached, mask_cached

        fill_rows = self.texture_fill_rows(texture)
        if fill_rows is None:
            return None, None

        rgb = np.zeros((texture.height, texture.width, 3), dtype=np.uint8)
        ...
        self.texture_rgb_cache[texture_id] = rgb
        self.texture_mask_cache[texture_id] = mask
        return rgb, mask

    def screen_x_indices(self, width: int):
        if np is None:
            return None
        cached = self.screen_x_cache.get(width)
        if cached is not None:
            return cached
        values = np.arange(width, dtype=np.float32)
        self.screen_x_cache[width] = values
        return values

    def camera_x_values(self, width: int):
        if np is None:
            return None
        cached = self.camera_x_cache.get(width)
        if cached is not None:
            return cached
        if width <= 1:
        ...
        self.camera_x_cache[width] = values
        return values

    def vertical_sample_indices(self, line_length: int, tex_height: int):
        if np is None:
            return None
        key = (line_length, tex_height)
        cached = self.vertical_sample_cache.get(key)
        if cached is not None:
            return cached
        if line_length <= 1:
            values = np.zeros((1,), dtype=np.int32)
        else:
            values = np.linspace(0, max(0, tex_height - 1), line_length, dtype=np.float32).astype(np.int32)
        self.vertical_sample_cache[key] = values
        return values
    ...
    def draw_scene_surface_python(self) -> None:
        ...
        vw = self.view_width_cells
        vh = self.view_height_cells
        horizon = max(6, min(vh - 6, horizon))
        surface.fill(BACKGROUND)

        floor_rows = self.texture_fill_rows(floor_texture)
        ceiling_rows = self.texture_fill_rows(ceiling_texture)
        wall_fill_rows = {
            cell: self.texture_fill_rows(texture)
            for cell, texture in wall_textures.items()
        }

        left_ray_x = dir_x - plane_x
        left_ray_y = dir_y - plane_y
        right_ray_x = dir_x + plane_x
        right_ray_y = dir_y + plane_y
        ray_span_divisor = max(1, vw - 1)
        camera_plane_height = vh * 0.46

        color_cache: Dict[RGB, int] = {}

        def map_color(color: RGB) -> int:
            mapped = color_cache.get(color)
            if mapped is None:
                mapped = surface.map_rgb(color)
                color_cache[color] = mapped
            return mapped

        def wrapped_ratio(value: float) -> float:
            return value - math.floor(value)

        pixels = pygame.PixelArray(surface)
        try:
            for y in range(0, horizon):
                row_offset = max(0.5, float(horizon - y))
                row_distance = camera_plane_height / row_offset
                shade = max(0.22, min(0.96, 0.92 - row_distance * 0.065))
                world_x = cam_x + row_distance * left_ray_x
                world_y = cam_y + row_distance * left_ray_y
                step_x = row_distance * (right_ray_x - left_ray_x) / ray_span_divisor
                step_y = row_distance * (right_ray_y - left_ray_y) / ray_span_divisor
                base_fill: RGB = (16, 21, 27)

                if ceiling_rows is None or ceiling_texture is None or ceiling_texture.width <= 0 or ceiling_texture.height <= 0:
                    row_fill = map_color(self.shade_color(base_fill, shade))
                    for x in range(vw):
                        pixels[x, y] = row_fill
                else:
                    tex_w = ceiling_texture.width
                    tex_h = ceiling_texture.height
                    for x in range(vw):
                        tx = min(tex_w - 1, max(0, int(wrapped_ratio(world_x) * tex_w)))
                        ty = min(tex_h - 1, max(0, int(wrapped_ratio(world_y) * tex_h)))
                        sampled = ceiling_rows[ty][tx] or base_fill
                        pixels[x, y] = map_color(self.shade_color(sampled, shade))
                        world_x += step_x
                        world_y += step_y

            horizon_fill = map_color(self.shade_color(self.color_for(4), 0.90))
            for x in range(vw):
                pixels[x, horizon] = horizon_fill

            for y in range(horizon + 1, vh):
                row_offset = max(0.5, float(y - horizon))
                row_distance = camera_plane_height / row_offset
                shade = max(0.26, min(1.08, 1.02 - row_distance * 0.070))
                world_x = cam_x + row_distance * left_ray_x
                world_y = cam_y + row_distance * left_ray_y
                step_x = row_distance * (right_ray_x - left_ray_x) / ray_span_divisor
                step_y = row_distance * (right_ray_y - left_ray_y) / ray_span_divisor
                base_fill: RGB = self.floor_band_color(y)

                if floor_rows is None or floor_texture is None or floor_texture.width <= 0 or floor_texture.height <= 0:
                    row_fill = map_color(self.shade_color(base_fill, shade))
                    for x in range(vw):
                        pixels[x, y] = row_fill
                else:
                    tex_w = floor_texture.width
                    tex_h = floor_texture.height
                    for x in range(vw):
                        tx = min(tex_w - 1, max(0, int(wrapped_ratio(world_x) * tex_w)))
                        ty = min(tex_h - 1, max(0, int(wrapped_ratio(world_y) * tex_h)))
                        sampled = floor_rows[ty][tx]
                        if sampled is None:
                            fill = base_fill
                        else:
                            fill = self.blend_colors(base_fill, sampled, 0.86)
                        pixels[x, y] = map_color(self.shade_color(fill, shade))
                        world_x += step_x
                        world_y += step_y

            for x in range(vw):
                camera_x = 2.0 * x / max(1, vw - 1) - 1.0
                ...
                line_height = int((vh * 0.92) / max(0.001, distance))
                draw_start = max(1, horizon - line_height // 2)
                draw_end = min(vh - 3, horizon + line_height // 2)
                ...
                fill_rows = wall_fill_rows.get(cell)

                if texture is not None and fill_rows is not None and texture.width > 0 and texture.height > 0:
                    tex_w = texture.width
                    tex_h = texture.height
                    tx = min(tex_w - 1, max(0, int(min(0.999999, max(0.0, wall_hit)) * tex_w)))
                    for y in range(draw_start, draw_end + 1):
                        y_ratio = (y - draw_start) / max(1, draw_end - draw_start)
                        ty = min(tex_h - 1, max(0, int(y_ratio * max(1, tex_h - 1))))
                        sampled = fill_rows[ty][tx]
                        if sampled is None:
                            sampled = self.color_for(color_id)
                        pixels[x, y] = map_color(self.shade_color(sampled, shade))
                else:
    ...
    def draw_scene_surface_numpy(self) -> None:
        ...
        floor_rgb, floor_mask = self.texture_rgb_arrays(floor_texture)
        ceiling_rgb, ceiling_mask = self.texture_rgb_arrays(ceiling_texture)
        wall_rgb_arrays = {cell: self.texture_rgb_arrays(texture) for cell, texture in wall_textures.items()}

        arr = pygame.surfarray.pixels3d(surface)
        try:
            arr[:, :, :] = BACKGROUND
            x_steps = self.screen_x_indices(vw)
            camera_x_values = self.camera_x_values(vw)
            if x_steps is None or camera_x_values is None:
                return
            ...
            right_ray_y = dir_y + plane_y
            ray_span_divisor = max(1, vw - 1)
            span_dx = (right_ray_x - left_ray_x) / ray_span_divisor
            span_dy = (right_ray_y - left_ray_y) / ray_span_divisor
            camera_plane_height = vh * 0.46

            if horizon > 0:
                top_rows = np.arange(0, horizon, dtype=np.int32)
                if top_rows.size > 0:
                    top_offsets = np.maximum(0.5, (horizon - top_rows).astype(np.float32))
                    top_dist = (camera_plane_height / top_offsets).astype(np.float32)
                    top_shade = np.clip(0.92 - top_dist * 0.065, 0.22, 0.96).astype(np.float32)
                    wx = cam_x + top_dist[:, None] * (left_ray_x + span_dx * x_steps[None, :])
                    wy = cam_y + top_dist[:, None] * (left_ray_y + span_dy * x_steps[None, :])
                    if ceiling_rgb is None or ceiling_mask is None or ceiling_texture is None or ceiling_texture.width <= 0 or ceiling_texture.height <= 0:
                        ceiling_base = np.array((16, 21, 27), dtype=np.float32)
                        top_colors = np.broadcast_to(ceiling_base, (top_rows.size, vw, 3)).copy()
                    else:
                        tx = np.minimum(ceiling_texture.width - 1, (np.mod(wx, 1.0) * ceiling_texture.width).astype(np.int32))
                        ty = np.minimum(ceiling_texture.height - 1, (np.mod(wy, 1.0) * ceiling_texture.height).astype(np.int32))
                        top_colors = ceiling_rgb[ty, tx].astype(np.float32)
                        sampled_mask = ceiling_mask[ty, tx]
                        if not np.all(sampled_mask):
                            ceiling_base = np.array((16, 21, 27), dtype=np.float32)
                            top_colors = np.where(sampled_mask[..., None], top_colors, ceiling_base[None, None, :])
                    top_colors *= top_shade[:, None, None]
                    arr[:, top_rows, :] = np.swapaxes(np.clip(top_colors, 0, 255).astype(np.uint8), 0, 1)

            arr[:, horizon, :] = np.array(self.shade_color(self.color_for(4), 0.90), dtype=np.uint8)

            if horizon + 1 < vh:
                bottom_rows = np.arange(horizon + 1, vh, dtype=np.int32)
                if bottom_rows.size > 0:
                    bottom_offsets = np.maximum(0.5, (bottom_rows - horizon).astype(np.float32))
                    bottom_dist = (camera_plane_height / bottom_offsets).astype(np.float32)
                    bottom_shade = np.clip(1.02 - bottom_dist * 0.070, 0.26, 1.08).astype(np.float32)
                    wx = cam_x + bottom_dist[:, None] * (left_ray_x + span_dx * x_steps[None, :])
                    wy = cam_y + bottom_dist[:, None] * (left_ray_y + span_dy * x_steps[None, :])
                    base_rows = np.array([self.floor_band_color(int(y)) for y in bottom_rows], dtype=np.float32)
                    if floor_rgb is None or floor_mask is None or floor_texture is None or floor_texture.width <= 0 or floor_texture.height <= 0:
                        bottom_colors = np.broadcast_to(base_rows[:, None, :], (bottom_rows.size, vw, 3)).copy()
                    else:
                        tx = np.minimum(floor_texture.width - 1, (np.mod(wx, 1.0) * floor_texture.width).astype(np.int32))
                        ty = np.minimum(floor_texture.height - 1, (np.mod(wy, 1.0) * floor_texture.height).astype(np.int32))
                        sampled = floor_rgb[ty, tx].astype(np.float32)
                        sampled_mask = floor_mask[ty, tx]
                        blended = base_rows[:, None, :] * 0.14 + sampled * 0.86
                        bottom_colors = np.where(sampled_mask[..., None], blended, base_rows[:, None, :])
                    bottom_colors *= bottom_shade[:, None, None]
                    arr[:, bottom_rows, :] = np.swapaxes(np.clip(bottom_colors, 0, 255).astype(np.uint8), 0, 1)

            for x in range(vw):
                camera_x = float(camera_x_values[x])
                ray_dir_x = dir_x + plane_x * camera_x
                ray_dir_y = dir_y + plane_y * camera_x
                distance, cell, side, wall_hit = dungeona.cast_perspective_ray(grid, cam_x, cam_y, ray_dir_x, ray_dir_y)
                if cell not in {"#", "D"}:
                ...
                texture = wall_textures.get(cell)
                texture_rgb, texture_mask = wall_rgb_arrays.get(cell, (None, None))
                line_length = draw_end - draw_start + 1

                if texture is not None and texture_rgb is not None and texture_mask is not None and texture.width > 0 and texture.height > 0 and line_length > 0:
                    tx = min(texture.width - 1, max(0, int(min(0.999999, max(0.0, wall_hit)) * texture.width)))
                    ty_indices = self.vertical_sample_indices(line_length, texture.height)
                    column = texture_rgb[ty_indices, tx].astype(np.float32)
                    sampled_mask = texture_mask[ty_indices, tx]
                    if not np.all(sampled_mask):
                        default_color = np.array(self.color_for(color_id), dtype=np.float32)
                        column = np.where(sampled_mask[:, None], column, default_color[None, :])
                    column *= shade
                    arr[x, draw_start : draw_end + 1, :] = np.clip(column, 0, 255).astype(np.uint8)
                else:
                    fill_color = self.color_for(color_id)
                    if cell == "D":
                        fill_color = self.shade_color(fill_color, 0.92)
                    arr[x, draw_start : draw_end + 1, :] = np.array(self.shade_color(fill_color, shade), dtype=np.uint8)

                ceiling_limit = max(1, draw_start - 1)
                if ceiling_limit > 1 and x % 3 == 0:
                    edge_fill = self.char_fill("_", 4)
                    if edge_fill is not None:
                        arr[x, ceiling_limit, :] = np.array(edge_fill, dtype=np.uint8)
        finally:
            del arr

    def draw_scene_surface(self) -> None:
        if np is not None:
            self.draw_scene_surface_numpy()
        else:
            self.draw_scene_surface_python()

    # ---------- UI drawing ----------
    def draw_scaled_view(self) -> None:
        assert self.view_surface is not None
        assert self.scaled_view_surface is not None
        vx, vy, vw, vh = self.view_rect
        if self.scaled_view_surface.get_size() != (vw, vh):
            self.scaled_view_surface = pygame.Surface((vw, vh)).convert()
        pygame.transform.scale(self.view_surface, (vw, vh), self.scaled_view_surface)
        self.screen.blit(self.scaled_view_surface, (vx, vy))

    def draw_ansi_billboards(self) -> None:
        for distance, screen_offset, kind, texture in self.gather_visible_billboards():
            if kind == "grail":
                object_projection_scale = 5.0
    ...
    def handle_keydown(self, event: "pygame.event.Event") -> bool:
        ...
        if self.state.get("show_congrats_banner") and key not in {pygame.K_ESCAPE, pygame.K_F12}:
        ...
        if key in {pygame.K_UP, pygame.K_w}:
            ...
            recenter_after_step = moved
            acted = True
        elif key in {pygame.K_a, pygame.K_z}:
            old_pos = (self.state["x"], self.state["y"])
            ...
            recenter_after_step = moved
            acted = True
        elif key in {pygame.K_d, pygame.K_c}:
            old_pos = (self.state["x"], self.state["y"])
            dungeona.try_strafe(self.state, 1)
            moved = old_pos != (self.state["x"], self.state["y"])
            ...
            self.state["message"] = dungeona.travel_stairs(self.state, -1)
            acted = True
        elif key in {pygame.K_ESCAPE, pygame.K_F12}:
            return False
        ...
        while running:
            for event in pygame.event.get():
                if event.type == pygame.QUIT:
                    ...
                    self.display_surface = self.create_display_surface(new_w, new_h)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Play Dungeona in a pygame / SDL window with optional scaled-display GPU presentation.")
    ...
    parser.add_argument("--no-gpu", action="store_true", help="Disable pygame SCALED/vsync presentation and use the plain display surface.")
...
def main() -> int:
    ...
    app = Dungeona3(args.width, args.height, fullscreen=args.fullscreen, fps=args.fps, use_gpu=not args.no_gpu)
    return app.run()
